...
기본 카탈로그에는 CMU Graphics Lab Motion Capture Database 기반 BVH **2,224종**이 걷기·달리기·점프·춤·발차기·동물 흉내·공구 작업 등 **48개 동작 분류**로 정리되어 있습니다. API 키나 유료 외부 모델은 필요하지 않습니다.
...
1. 맨 위 **검색** 칸에 `걷기`, `walk`, `점프`, `발차기` 같은 단어를 입력합니다. 입력하는 동안 목록이 바로 좁혀지며, 비우면 2,224종 전체가 보입니다. 한국어 분류 이름과 영어 원본 설명 단어가 모두 태그로 들어 있고, `141_19` 같은 파일 이름으로도 찾습니다. 아래 줄의 **카테고리** 목록으로 분류를 고르거나 **받은 모션만** 토글로 이미 내려받은 것만 남길 수 있습니다.
...
설명이 있는 동작은 버리지 않습니다. 위 분류에 하나도 걸리지 않으면 `기타`로 들어가므로, 저장소의 BVH 2,224개가 모두 이름과 분류를 갖습니다. CMU 인덱스가 `Unknown`·`clean`처럼 의미 없는 설명만 주거나 설명이 아예 없는 항목은 이름을 만들 수 없어 제외합니다.
