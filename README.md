...
CatAni 0.6.4는 공개 모션 캡처 데이터를 Blender 안에서 **검색하고, 뷰포트에서 재생해 확인하고, 캐릭터에 바로 적용**하는 Extension입니다. 사이드바에 검색 칸과 긴 목록을 그대로 두어 샘플을 훑으면서 고릅니다. 폴더 경로·출처·이용 조건·체크섬·굽기 옵션 같은 정보는 별도 팝업으로 옮겼습니다.
...
1. [Releases](https://github.com/zzamjak-cloud/CatAni-Blender/releases/latest)에서 `catani-v0.6.4.zip`을 내려받습니다.
2. **Edit → Preferences → Get Extensions → ▼ → Install from Disk**를 누릅니다.
3. 내려받은 ZIP을 선택합니다.

같은 페이지의 `catani-v0.6.4.zip.sha256`으로 무결성을 확인할 수 있습니다.

```bash
shasum -a 256 -c catani-v0.6.4.zip.sha256   # macOS / Linux
```
...
8. 패널 아래 한 줄에 결과가 나옵니다. 예: `적용됨 · 부위 22/22 · 2~466f · 키 3,303개(-92%) · 방향 오차 1.79°`. 재생하면 캐릭터가 그 동작을 합니다.
...
7. 결과를 새 Action에 **베지어** 키로 굽습니다. `곡선 간소화 오차`(기본 1.0°) 안에서 3차 베지어 구간을 **최소제곱으로 적합**하므로, 프레임마다 선형 키가 박힌 곡선이 아니라 손으로 고칠 수 있는 곡선이 나옵니다. 쪼갠 뒤에는 **이웃 구간을 다시 합쳐 보는 패스**가 하향식 쪼개기가 남긴 불필요한 매듭을 걷어냅니다. 실제 CMU 걷기에서 키가 42,319개에서 3,303개(**92% 감소**)로 줄었습니다. 0으로 두면 모든 프레임에 키를 남깁니다.
...
| 0.5° | 5,657 | 87% | 0.72° | 0.004 |
| **1.0°(기본)** | **3,303** | **92%** | **1.79°** | 0.008 |
| 2.0° | 1,909 | 95% | 4.13° | 0.015 |

같은 데이터에 키를 표본 프레임에만 놓는 방식을 쓰면 0.5°에서 18,872개, 1.0°에서 12,715개가 남습니다. 적합 쪽이 70~74% 더 적습니다.

`노이즈 완화`(기본 끔)는 모캡 흔들림을 프레임 창으로 걷어냅니다. 다만 실측에서는 손익이 나빴습니다. 완화 3프레임 + 0.5°(3,628키 · 2.21°)보다 완화 없이 1.0°(3,303키 · 1.79°)가 키도 적고 오차도 작습니다. 허용치를 키우는 편이 항상 낫기 때문에 기본으로 끄고, 유독 지저분한 캡처에만 쓰도록 남겨 두었습니다.
...
매 프레임 키를 만들지 않습니다. 굽는 단계에서 회전 곡선을 3차 베지어 최소제곱으로 적합하므로, 실제 CMU 걷기에서 키가 42,319 → 3,303개(92% 감소)로 남습니다.
...
현재 공개 버전은 **0.6.4**입니다. 새 버전을 준비할 때는 Python 3.11 이상과 Blender 5.2.0 이상을 갖추고 저장소 루트에서 실행합니다.

```bash
python3 scripts/validate_release.py --tag v0.6.4
python3 scripts/run_tests.py
python3 scripts/package.py --tag v0.6.4
```
...
`dist/catani-v0.6.4.zip`과 SHA-256 파일이 생성됩니다. 패키지에는 런타임 Python 파일, 매니페스트, GPL 라이선스와 기본 데모 BVH만 포함합니다. 샘플 `.blend`, 테스트, 개발 프로필, 계정 정보, 다운로드된 CMU 모션 파일은 포함하지 않습니다. 소스와 ZIP 모두 Blender 공식 Extension 검증을 거칩니다. 로컬 ZIP은 별도 검증용 Blender 프로필의 **Install from Disk**로 설치할 수 있습니다.
...
공개 저장소는 [`zzamjak-cloud/CatAni-Blender`](https://github.com/zzamjak-cloud/CatAni-Blender)입니다. `v0.5.0`·`v0.5.1`·`v0.6.0`·`v0.6.2`·`v0.6.3`·`v0.6.4` Release, GitHub Pages 원격 저장소가 모두 게시되어 있습니다.
...
v0.6.3을 깨끗한 인수 프로필에서 확인했습니다: 원격 저장소 등록 → v0.6.2 설치 → 동기화 → `catani [outdated: 0.6.2 -> 0.6.3]` 확인 → 업데이트 → 매니페스트 `0.6.3` → 활성화와 연산자 14개 등록 → 카탈로그 5,908종 로드(상업 사용 가능 2,831 + 동봉 예제 1 / 비상업 3,077)와 `정면 정렬` 기본 켜짐 확인. Release 자산의 SHA-256은 내려받은 바이트, 함께 올린 `.sha256`, `index.json`의 `archive_hash` 세 곳에서 모두 `83ccddbb…`로 일치했습니다. 로컬 `scripts/package.py` 산출물(`7261ef57…`)은 같은 내용이지만 압축 구현 차이로 해시가 다르므로, 배포 검증은 항상 Release 자산 쪽 값을 씁니다.
