...
기본 카탈로그에는 BVH **5,610종**이 걷기·달리기·점프·춤·발차기·동물 흉내·공구 작업 등 **50개 동작 분류**로 정리되어 있습니다. 상업 사용이 가능한 CMU Graphics Lab 2,533종(저장소 전량)과, 감정·연령 스타일이 붙은 Bandai Namco Research 3,077종(**CC BY-NC · 비상업 전용**)입니다. 비상업 데이터는 기본으로 숨어 있고, 켜면 목록에 `NC` 배지로 구분됩니다. API 키나 유료 외부 모델은 필요하지 않습니다.
