...
v0.6.0을 깨끗한 인수 프로필에서 확인했습니다: 원격 저장소 등록 → 동기화 → 설치 → 활성화 → 매니페스트 `0.6.0` → 카탈로그 5,908종·50분류 로드(상업 사용 가능 2,831 / 비상업 3,077) → 한국어·영어 검색(`발차기` 37건, `walk` 715건, `동물` 138건). Release 자산의 SHA-256은 내려받은 바이트, 함께 올린 `.sha256`, `index.json`의 `archive_hash` 세 곳에서 모두 `efb609e1…`로 일치했습니다. 로컬 `scripts/package.py` 산출물은 같은 내용(멤버 11개의 크기·CRC 일치)이지만 압축 구현 차이로 해시가 다르므로, 배포 검증은 항상 Release 자산 쪽 값을 씁니다.
