...
CatAni 0.5.1은 공개 모션 캡처 데이터를 Blender 안에서 **검색하고, 표본 포즈로 확인하고, 캐릭터에 바로 적용**하는 Extension입니다. 사이드바에는 `모션 샘플 보기` 버튼 하나만 두고, 검색·목록·미리보기·적용은 넓은 팝업에서 한 번에 합니다. 폴더 경로·출처·이용 조건·체크섬·굽기 옵션 같은 정보는 모두 별도 팝업으로 옮겼습니다.
...
1. **모션 샘플 보기**를 누르면 넓은 팝업이 열립니다. 좁은 사이드바 대신 여기서 고릅니다.
2. 팝업 위쪽의 **검색** 칸에 `걷기`, `walk`, `점프`, `발차기` 같은 단어를 입력합니다. 입력하는 동안 목록이 바로 좁혀지며, 비우면 485종 전체가 보입니다. 한국어 분류 이름과 영어 원본 설명 단어가 모두 태그로 들어 있습니다. 옆의 **카테고리** 목록으로 분류를 고르거나 **받은 모션만** 토글로 이미 내려받은 것만 남길 수 있습니다.
3. 왼쪽 **목록**에는 받아 둔 모션과 아직 받지 않은 공개 모션이 함께 나옵니다. 오른쪽에 `BVH`/`FBX`로 표시되면 로컬 파일이고, `받기 228KB`처럼 보이면 아직 내려받지 않은 공개 데이터입니다.
4. 항목을 고르면 오른쪽에 **표본 포즈 6장**이 시간 순서로 그려집니다. BVH를 직접 읽어 그린 뼈대 그림이라 파일을 씬에 불러오지 않고도 어떤 동작인지 바로 보입니다(흐린 선은 직전 표본). 프레임 수·길이·태그·설명도 함께 나옵니다.
5. **뷰포트에서 재생**을 누르면 모션 원본 리그를 임시로 불러와 실제 동작을 재생합니다. 아직 받지 않은 모션이면 먼저 내려받습니다. 확인이 끝나면 사이드바의 **미리보기 정리**로 임시 리그와 프레임 범위를 되돌립니다.
6. **대상**에 애니메이션을 적용할 캐릭터 아마추어를 지정합니다. 팝업과 사이드바 어느 쪽에서 지정해도 같습니다. 아마추어를 선택해 둔 상태면 비워 두어도 됩니다.
7. 팝업 아래 **적용하기**(또는 사이드바의 **적용**)를 누릅니다. 아직 받지 않은 모션이면 먼저 내려받아 체크섬을 검증하고, 이어서 자동으로 적용까지 진행합니다. 미리보기로 불러둔 리그가 있으면 다시 읽지 않고 그대로 씁니다.
8. 사이드바 아래 한 줄에 결과가 나옵니다. 예: `적용됨 · 부위 22/22 · 2~466f · 키 3,897개(-91%) · 방향 오차 4.21°`. 재생하면 캐릭터가 그 동작을 합니다.

`ⓘ 출처` 버튼은 선택 모션의 실제 다운로드 주소, git blob SHA-1, 출처, 이용 조건과 원문 링크를 팝업으로 보여 줍니다. `⚙ 상세` 버튼은 모션 폴더, 프레임 간격, 노이즈 완화, 곡선 간소화 오차, 이동 적용, 바닥 관통 보정, IK 전환, 원본 리그 숨기기와 **마지막 적용 리포트**(본 매핑 표, 규격, 키 수와 감소율, 검증 오차, 실측 곡선 오차, 접지 보정량, IK 체인과 목표·관절 오차, 음소거한 NLA 트랙)를 담습니다.
